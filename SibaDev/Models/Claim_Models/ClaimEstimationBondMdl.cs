...
{
    public class ClaimEstimationBondMdl : Model
    {
        ...
        public static CLM_EST_HEAD get_estimation(string code)
        {
            ...
            try
            {
                return (from r in db.CLM_EST_HEAD where r.CLM_N0 == code select r).ToList().Select(r => new CLM_EST_HEAD
                {
                    ...
                    CLM_EST_BONDS_RISK_DTLS = r.CLM_EST_BONDS_RISK_DTLS != null ? r.CLM_EST_BONDS_RISK_DTLS.Select(c => get_risk(c.CLM_RISK_SYS_ID)).ToList() : null,
            ...
            switch (polhClaim.CLM_STATUS)
            {
                case "A":
                    {
                        ...
                        if (dbProd != null)
                        {
                             ...
                             * ESTIMATION RISK
                             *-------------------------------*/
                            foreach (var risk in polhClaim.CLM_EST_BONDS_RISK_DTLS)
                            {
                                ...
                                switch (risk.CLM_RISK_STATUS)
                                {
                                    case "A"://we have a risk in the database which we have to update
                                        var dbriskdtls = db.CLM_EST_BONDS_RISK_DTLS.Find(risk.CLM_RISK_SYS_ID);
                                        db.CLM_EST_BONDS_RISK_DTLS.Attach(dbriskdtls);
                                        dbriskdtls.Map(risk);
                                        break;
                                    case "U"://add new risk
                                        risk.CLM_RISK_CLM_SYS_ID = polhClaim.CLM_SYS_ID;
                                        ...
                                        db.CLM_EST_BONDS_RISK_DTLS.Add(risk);
                                        break;
                                    case "D"://                                       
                                        db.CLM_EST_BONDS_RISK_DTLS.Attach(db.CLM_EST_BONDS_RISK_DTLS.Find(risk.CLM_RISK_SYS_ID));
                ...
                case "U":

                    {
                        ...
                        polhClaim.CLM_EST_BONDS_RISK_DTLS.ForEach(x => x.CLM_RISK_STATUS = "A");
