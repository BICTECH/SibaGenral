(function ($, s, u, $scope) {
    $(function () {
        ...
        ['Description', 'Principal', 'Sum Insured', 'Premium', 'Status', 'Created By', 'Created Date', 'ID', 'Claim ID'],
       [
          { name: 'CLM_RISK_DESCRIPTION', id: false, index: 'CLM_RISK_DESCRIPTION' },
          { name: 'CLM_RISK_PRINCIPAL', id: false, index: 'CLM_RISK_PRINCIPAL', width: 150 },
          { name: 'CLM_RISK_SI', id: false, index: 'CLM_RISK_SI', width: 150 },
          { name: 'CLM_RISK_PRM', id: false, index: 'CLM_RISK_PRM', width: 150 },
      ...
      function (sel_id) {
        ...
        ['ID', 'Description', 'Principal', 'Sum Insured', 'Premium', 'Status', 'Created By', 'Created Date'],
       [
            ...
            s.getRisksLOV(RisksData, function (result) {
                if (result && result.length && result.length > 0) {
                    ...
                    for (var i in result) {
                        ...
                        $scope.polrisk_grid.addRowData(result[i]["BOND_SYS_ID"], result[i]);
                    }
                }
            });
        }
      ...
      */
        ...
        $("#btn_Save").on('click', function () {
            ...
            u.modal_confirmation("Are you sure you want to save?", function (e) {
                ...
                productData.CLM_EST_BONDS_RISK_DTLS = u.get_grid_data($scope.riskdtls_grid);
        ...
        *----------------*/
        ...
        $scope.search_fill = function (product) {
            ...
            u.clear_grid_data($scope.riskdtls_grid);
            for (var i in product.CLM_EST_BONDS_RISK_DTLS) {
                $scope.riskdtls_grid.addRowData(product.CLM_EST_BONDS_RISK_DTLS[i].CLM_RISK_SYS_ID, product.CLM_EST_BONDS_RISK_DTLS[i]);
            }
            //populating child entities
            u.clear_grid_data($scope.estDtls_grid);
