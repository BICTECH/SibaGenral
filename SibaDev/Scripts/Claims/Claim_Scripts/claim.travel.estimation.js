(function ($, s, u, $scope) {
    $(function () {
        ...
        ['Insure Name', 'Nationality', 'National ID', ' Passport No.', 'Country Visited', 'Destination Address', 'Destination Phone No.', 'Status', 'Created By', 'Created Date', 'ID', 'Claim ID'],
       [
          { name: 'CLM_RISK_CLT_NAME', id: false, index: 'CLM_RISK_CLT_NAME' },
          { name: 'CLM_RISK_NAT', id: false, index: 'CLM_RISK_NAT' },
          { name: 'CLM_RISK_NAT_ID', id: false, index: 'CLM_RISK_NAT_ID', width: 150 },
          { name: 'CLM_RISK_PASSPORT_NO', index: 'CLM_RISK_PASSPORT_NO', width: 200 },
          { name: 'CLM_RISK_COUNTRY_VISITED', index: 'CLM_RISK_COUNTRY_VISITED' },
          { name: 'CLM_RISK_DESTIN_ADDRSS', index: 'CLM_RISK_DESTIN_ADDRSS' },
          { name: 'CLM_RISK_PHONE_NO', index: 'CLM_RISK_PHONE_NO' },
      ...
      function (sel_id) {
                ...
                $scope.polrisk_grid = u.default_grid("#grdPolRisk", "#grdPolRiskPager", "Claim Risk List",
        ['ID', 'Insure Name', 'Nationality', 'National ID', ' Passport No.', 'Country Visited', 'Destination Address', 'Destination Phone No.', 'Status', 'Created By', 'Created Date'],
       [
            ...
            s.getRisksLOV(RisksData, function (result) {
                if (result && result.length && result.length > 0) {
                    ...
                    for (var i in result) {
                        ...
                        $scope.polrisk_grid.addRowData(result[i]["TRA_SYS_ID"], result[i]);
                    }
                }
            });
        }
      ...
      */
        ...
        $("#btn_Save").on('click', function () {
            ...
            u.modal_confirmation("Are you sure you want to save?", function (e) {
                ...
                productData.CLM_EST_TRAVEL_RISK_DTLS = u.get_grid_data($scope.riskdtls_grid);
        ...
        *----------------*/
        ...
        $scope.search_fill = function (product) {
            ...
            u.clear_grid_data($scope.riskdtls_grid);
            for (var i in product.CLM_EST_TRAVEL_RISK_DTLS) {
                $scope.riskdtls_grid.addRowData(product.CLM_EST_TRAVEL_RISK_DTLS[i].CLM_RISK_SYS_ID, product.CLM_EST_TRAVEL_RISK_DTLS[i]);
            }
            //populating child entities
            u.clear_grid_data($scope.estDtls_grid);
