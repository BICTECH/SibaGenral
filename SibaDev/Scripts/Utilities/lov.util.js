(function ($, s, u, $scope) {
    u.add({
        ...
        row_single_click: function (id) {
            var rowData = $scope.rowData = $(this).getRowData(id);
            $("form input[name='" + $scope.dialog_data.field2 + "'], form textarea[name='" + $scope.dialog_data.field2 + "']").val(rowData.NAME);
            $("form input[name='" + $scope.dialog_data.field1 + "'], form textarea[name='" + $scope.dialog_data.field1 + "']").val(rowData.CODE);
        },
        ...
        lovDropDown: function (id, service) {
            if (s[service]) {
                s[service](function (result) {
                    for (var i in result) {
                        $(id).append("<option value='" + result[i]["CODE"] + "'>" + result[i]["NAME"] + "</option>");
                    }
                })
            } else {
