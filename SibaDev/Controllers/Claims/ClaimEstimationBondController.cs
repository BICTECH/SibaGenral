...
{
    public class ClaimEstimationBondController : ApiController
    {
        // GET: api/ClaimEstimation

        [HttpPost]
        [Route("api/ClaimEstimationBond/save")]
        public object Save([FromBody] CLM_EST_HEAD estimations)
        {
            try
            {
                ClaimEstimationBondMdl.save_estimation(estimations);
        ...
        [HttpGet]
        [Route("api/ClaimEstimationBond/search/{q}")]
        public object Search(string q)
        {
            try
            {
                return ClaimEstimationBondMdl.search_estimations(q);
            }
            catch (Exception e)
            {
